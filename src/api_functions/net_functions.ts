...
import type { network_check_result } from "../types.js";
...
export function get_clal(cookieArr: string[]): { cookie: string } | undefined {
  ...
  if (extracted_cookie) {
    const extractedParts: string[] = extracted_cookie
      .split(";")
    ...
    return {
      cookie: clalValue as string,
    };
  } else return;
}

export async function networkCheck(): Promise<network_check_result> {
...
export async function NET_login(clal_cookie: string): Promise<string> {
  // initial login
  let currentFetch: Response = await fetch(LOGIN_PAGE, {
    method: "GET",
    headers: {
      ...DEFAULT_HEADERS,
  ...
  while (currentFetch.headers.get("location") !== END_PAGE) {
    currentFetch = await fetch(currentFetch.headers.get("location") as string, {
      method: "GET",
      headers: {
        ...DEFAULT_HEADERS,
